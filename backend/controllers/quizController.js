  ...
  try {
    ...
    try {
      ...
      if (isValidQuizQuestions(parsed)) {
        ...
        // Cache the correct answers server-side with a session timestamp
        // so we can periodically clean up stale entries
        ...
        // Periodically clean up stale cache entries (older than 1 hour)
    ...
    res.status(500).json({
      success: false,
      message: error.message || "Failed to get sample questions.",
    });
  }
};
  ...
  try {
    ...
    if (!subject || !questions || !answers) {
      return res.status(400).json({
        success: false,
        message: "Subject, questions, and answers are required.",
      });
    }

    // Check both the global cache (Gemini questions) and the hardcoded bank
    const pool = questionBank[subject] || questionBank.Physics;
    ...
    const questionResults = questions.map((q, i) => {
      ...
      if (cache.has(q.question)) {
        ...
        if (bankQuestion) {
          correctIndex = bankQuestion.correct;
        }
      }

      // Reject questions we can't validate
